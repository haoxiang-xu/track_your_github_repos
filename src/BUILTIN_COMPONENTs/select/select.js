...
const Select = ({
  ...
  const baseHeight =
    ...
    theme?.input?.height ??
    baseFontSize + 16;
  const baseColor = style?.color ?? select_theme?.color ?? theme?.color ?? "black";
  ...
  const blurOutline =
    ...
    "1px solid #CCCCCC";
  const outlineValue =
    style?.outline || (isTriggerFocused || mergedOpen ? focusOutline : blurOutline);
  ...
  const triggerContent = (
    <div
      ...
      style={{
        ...
        padding: style?.padding ?? `${default_top_bottom_padding}px ${default_left_right_padding}px`,
        height: baseHeight,
